...
class ParquetSampler:
    def __init__(
        ...
        if self.verbose:
            print("\nBuilding or loading file index...")
        self.file_index = self.build_or_load_file_index()
    ...
    def get_cache_filename(self):
        # Create a unique filename based on the data directories and their modification times
        dir_info = []
        for directory in self.data_dirs:
            dir_mtime = os.path.getmtime(directory)
            dir_info.append(f"{directory}:{dir_mtime}")
        dir_string = ",".join(dir_info)
        hash_object = hashlib.md5(dir_string.encode())
        return f"parquet_index_cache_{hash_object.hexdigest()}.json"

    def build_or_load_file_index(self) -> List[Tuple[str, int, int]]:
        if self.index_cache_dir:
            os.makedirs(self.index_cache_dir, exist_ok=True)
            cache_file = os.path.join(self.index_cache_dir, self.get_cache_filename())
            
            if os.path.exists(cache_file):
                if self.verbose:
                    print(f"Loading index from cache: {cache_file}")
                with open(cache_file, 'r') as f:
                    return [(entry[0], entry[1], entry[2]) for entry in json.load(f)]
...
def main():
    ...
    parser.add_argument("--index_cache_dir", type=str, default=None, help="Directory to store the index cache")
    ...
    if args.verbose:
        ...
        print(f"Index cache directory: {args.index_cache_dir}")
        print("-----------------------------\n")
