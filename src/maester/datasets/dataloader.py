# Modified from IBM's FMS-FSDP repository, released under Apache 2.0.
import csv
import os
import math
import random
...
class _StatefulDataset(IterableDataset, Stateful):
    """
    ...
    def _reshard(self, sharded_list: List[List[Any]]):
        """
...
class StreamingDocDataset(_StatefulDataset):
    ...
    StreamingDocDataset uses integer weights to implement dataset weighting via oversampling per-epoch.
    ...
    def __init__(
        ...
        super(StreamingDocDataset, self).__init__(rank, worldsize)
        self.seed = seed
        self.data = datapath
...
class ScalableShardDataset(_StatefulDataset):
    """
...
class SamplingDataset(_StatefulDataset):
    """
    ...
    def __init__(
        self,
        datapath: str,
        dataset: Type["StreamingDocDataset"] | Type["ScalableShardDataset"],
        rank: int,
        worldsize: int,
        delimiter_token: Any,
        datasets=None,
