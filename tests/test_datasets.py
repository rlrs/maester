...
def test_weight():
    """
    A test for base, non-wrapper datasets using oversampling (currently StreamingDoc and ScalableShard).
...
def test_multi_reload_stress():
    ...
    d7 = lambda x: [PreloadBufferDataset(d, 99) for d in x]
...
def test_scalable_partitioning():
    ...
    """
    for layer in [ScalableShardDataset, SamplingDataset]:
        kwargs = {
            "n_logical_shards": 12,
            "datasets": ["dataset_1"],
            "weights": [1],
        ...
        datasets = [
            layer(tmpdir.name, ScalableShardDataset, i, 4, -1, **kwargs)
            if layer == SamplingDataset
