...
class Sampling_Dataset(_Stateful_Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert len(data_dirs) > 0, "You must specify at least one dataset directory"

        if weights is not None:
            assert len(weights) == len(
                data_dirs
            ), f"Number of oversample weights {len(weights)} must match number of dataset directories {len(data_dirs)}"
            for w in weights:
                assert w > 0, f"Sampling rate {w} must be positive"
        self.weights = [1] * len(data_dirs) if weights is None else weights
        self.weights = [w / sum(self.weights) for w in self.weights]

        self.tokens_seen = [0] * len(data_dirs)
        ...
        self.iterators = [iter(d) for d in self.data]
