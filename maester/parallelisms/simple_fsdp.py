...
from torch.distributed.tensor import distribute_tensor, Partial, Replicate, Shard, DTensor
    ...
    def _fsdp_recomp_policy():
        def _custom_policy(ctx, func, *args, **kwargs):
            to_recompute = func in {
                ...
                torch.ops.aten._to_copy.default, # for dtype casts
            }
            return (
                CheckpointPolicy.MUST_RECOMPUTE if to_recompute else CheckpointPolicy.MUST_SAVE
            )
...
class ReplicateComputation(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if False: #self.regional_ac and self.mode in ("fully_shard", "hybrid_shard"):
...
def simple_fsdp(model, device_mesh, mode="replicate", ac_mode: str = "none", mp_policy: Optional[MixedPrecisionPolicy] = None):
    if mode == "replicate":
        ...
        assert device_mesh.ndim == 2, "Hybrid sharding requires a 2D device mesh"
    else:
        raise ValueError(f"Unsupported FSDP mode: {mode}")
    ...
    for mod in modules:
        ...
        for p_name, p in params_dict.items():
            if p is not None and p.numel() > 0:
                distribute_tensor_fn = _distribute_dtensor if isinstance(p, DTensor) else distribute_tensor
                mod.register_parameter(p_name, torch.nn.Parameter(distribute_tensor_fn(p, device_mesh, param_sharding)))
                # need a custom register_parametrization for DCP compatibility...
                register_parametrization(mod, p_name, ReplicateComputation(device_mesh, param_sharding, mode, regional_ac, mp_policy=mp_policy), unsafe=True)
        # _register_parametrization(mod, list(params_dict.keys()), ReplicateComputation(device_mesh, param_sharding, mode, regional_ac, mp_policy=mp_policy))
    return model
